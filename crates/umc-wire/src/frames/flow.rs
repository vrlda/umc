...
impl MaxDataFrame {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, FrameError> {
        let mut out = Vec::new();
        crate::varint::encode_into(&mut out, FrameType::MAX_DATA.0)
            .map_err(FrameError::VarintEncode)?;
        crate::varint::encode_into(&mut out, self.maximum_data)
            .map_err(FrameError::VarintEncode)?;
        Ok(out)
    }
...
impl MaxStreamDataFrame {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, FrameError> {
        let mut out = Vec::new();
        crate::varint::encode_into(&mut out, FrameType::MAX_STREAM_DATA.0)
            .map_err(FrameError::VarintEncode)?;
        crate::varint::encode_into(&mut out, self.stream_id).map_err(FrameError::VarintEncode)?;
        crate::varint::encode_into(&mut out, self.maximum_stream_data)
            .map_err(FrameError::VarintEncode)?;
        Ok(out)
    }
...
impl MaxStreamsFrame {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, FrameError> {
        let mut out = Vec::new();
        crate::varint::encode_into(&mut out, FrameType::MAX_STREAMS.0)
            .map_err(FrameError::VarintEncode)?;
        crate::varint::encode_into(&mut out, u64::from(!self.bidirectional))
            .map_err(FrameError::VarintEncode)?;
        crate::varint::encode_into(&mut out, self.maximum_streams)
            .map_err(FrameError::VarintEncode)?;
        Ok(out)
    }
...
mod tests {
    ...
    fn max_stream_data_round_trip() {
        let f = MaxStreamDataFrame {
            stream_id: 3,
            maximum_stream_data: 256 * 1024,
        };
        let enc = f.encode().unwrap();
    ...
    fn max_streams_round_trip_and_direction_validation() {
        ...
        assert_eq!(dec, f);
        assert_eq!(
            MaxStreamsFrame::decode(&[0x02, 0x10]).unwrap_err(),
