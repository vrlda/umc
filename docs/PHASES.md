...
| K | K1 implemented; K2 handshake core, K4 enumeration guard, K5 onion primitive, and K6 path policy primitive implemented; K3/K4 remainder–K8 pending | Privacy profiles are ordered and fail-safe by default (`p0`); a local policy may raise the effective profile and GetStatus/GetConfig expose it. ClientHello now binds a requested minimum into the capabilities hash, and the daemon rejects requests above its p1 maximum rather than downgrading. Control candidate enumeration now has a per-principal budget, `umc-relay` can build/open authenticated one-layer-at-a-time route envelopes, and sessions can reject direct paths when a private route policy is selected. Session-level profile reporting, DCID rotation, mesh-secret hints, daemon route wiring, and P3 padding remain future work. |
