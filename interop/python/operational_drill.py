...
def signing_drill(repo: Path, directory: Path) -> str:
    with tempfile.TemporaryDirectory(prefix="umc-ops-drill-") as temporary:
        ...
        trusted.mkdir()
        manifest.write_text('{"manifest_version":1,"release":"0.1.0"}\n', encoding="utf-8")
        ...
        manifest_with_signing.write_text(
            json.dumps({"manifest_version": 1, "release": "0.1.0", "signing": {"threshold": 1, "signatures": [{"key_id": key_id, "file": "manifest.sig"}]}}, sort_keys=True) + "\n",
            encoding="utf-8",
        )
